...
# User data script for GPU EC2 instances running RuvonInference.
#
# This script:
# 1. Updates the system and installs dependencies
# 2. Configures Docker and ECR authentication
# 3. Pulls the GPU Docker image from ECR
# 4. Runs the container with GPU support
...
ECR_REPOSITORY="ruvoninference-gpu"
CONTAINER_NAME="ruvoninference-gpu"
IMAGE_TAG="latest"
...
log "Starting RuvonInference GPU setup..."
...
log "Starting RuvonInference GPU container..."
docker run -d \
    --name $CONTAINER_NAME \
...
sleep 60  # GPU containers take longer to start

# Check if container is running
if docker ps | grep -q $CONTAINER_NAME; then
    log "SUCCESS: RuvonInference GPU container is running successfully!"
...
#!/bin/bash
# Simple monitoring script for RuvonInference GPU container
...
log "SUCCESS: RuvonInference GPU setup completed successfully!"
